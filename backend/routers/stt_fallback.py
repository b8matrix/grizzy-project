...
class GenerateTranscriptRequest(BaseModel):
    ...
    video_id: str
    model_size: str = Field(default="base", description="Whisper size: tiny, base, or small")
    force: bool = False
...
def _delete_stt_record(video_id: str):
    conn = get_db()
    conn.execute("DELETE FROM stt_transcripts WHERE video_id = ?", (video_id,))
    conn.commit()
    conn.close()


def _is_stale_processing(video_id: str) -> bool:
    """Jobs stuck in processing (e.g. server crash) for ~50+ minutes can be retried."""
    conn = get_db()
    row = conn.execute(
        """
        SELECT 1 FROM stt_transcripts
        WHERE video_id = ? AND status = 'processing'
        AND datetime(created_at) < datetime('now', '-50 minutes')
        """,
        (video_id,),
    ).fetchone()
    conn.close()
...
def _run_stt_pipeline(video_id: str, model_size: str):
    ...
    try:
        print(f"[STT] Pipeline started for video: {video_id}")
        ...
        print(f"[STT] Pipeline completed for video: {video_id} ({len(transcript)} segments)")

    except Exception as e:
        error_msg = f"{type(e).__name__}: {str(e)}"
        print(f"[STT] Pipeline FAILED for video: {video_id} — {error_msg}")
        traceback.print_exc()
...
async def generate_transcript(req: GenerateTranscriptRequest, background_tasks: BackgroundTasks):
    ...
    Use force=true to clear cache and re-run (e.g. after timeout or error).
    """
    _init_stt_table()

    if req.force:
        _delete_stt_record(req.video_id)

    existing = _get_stt_record(req.video_id)

    if existing and existing["status"] == "completed" and existing["transcript_json"]:
        return {
            "status": "completed",
    ...
    if existing and existing["status"] == "processing":
        if _is_stale_processing(req.video_id):
            _delete_stt_record(req.video_id)
        else:
